...
gust_strength_ratio = params['gust_params']['core_size']/params['gust_params']['strength']
...
with open(os.path.join(cases_directory,'run.sh'),"w") as run_file:
    ...
    for i,rg_iter in enumerate(Mg):
        for ii,Mt_iter in enumerate(Mt):
            run_file.write(f"rotor_gust_interaction.py --aero --acs --filt -input_geom geom.json -input_param param_{i*len(Mg)+ii}.json -observer_param observer_lgrid.json -acs_param acs_param.json -res_param mdof_geom_param.json\n")
