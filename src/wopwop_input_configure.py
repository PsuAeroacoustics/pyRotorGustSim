...
def wopwop_input_configure(geom_params,input_params,res_param,observer_params,acs_params,saved_params):
    ...
    if not 'nbTheta' in observer_params or 'nbx' in observer_params or 'xLoc' in observer_params:
        ...
        observer_in = ObserverIn(nt = saved_params['iterations'],Title='mic array',attachedTo = 'aircraft',tMin = saved_params['t'][-int(2*np.pi/saved_params['dpsi'])],tMax=saved_params['t'][-1],fileName='observer.ascii',highPassFrequency=observer_params['highPassFrequency'])

    elif observer_params['xLoc'] is not None:
        observer_in = ObserverIn(nt = saved_params['iterations'],Title='mic array',attachedTo = 'aircraft',tMin = saved_params['t'][-int(input_params['computational_params']['number_of_revs']*2*np.pi/saved_params['dpsi'])],tMax=saved_params['t'][-1],xLoc=observer_params['xLoc'],yLoc=observer_params['yLoc'],zLoc=observer_params['zLoc'],highPassFrequency = observer_params['highPassFrequency'])

    elif observer_params['nbTheta'] is not None:
        observer_in = ObserverIn(nt = saved_params['iterations'],Title='mic array',attachedTo = 'aircraft',tMin = saved_params['t'][-int(input_params['computational_params']['number_of_revs']*2*np.pi/saved_params['dpsi'])],tMax=saved_params['t'][-1],radius=observer_params['radius'],nbTheta=observer_params['nbTheta'],nbPsi=observer_params['nbPsi'],thetaMin=observer_params['thetaMin'],thetaMax=observer_params['thetaMax'],psiMin=observer_params['psiMin'],psiMax = observer_params['psiMax'],highPassFrequency = observer_params['highPassFrequency'])

    elif observer_params['nbx'] is not None:
        observer_in = ObserverIn(nt = saved_params['iterations'],Title='mic array',attachedTo = 'aircraft',tMin = saved_params['t'][-int(input_params['computational_params']['number_of_revs']*2*np.pi/saved_params['dpsi'])],tMax=saved_params['t'][-1],nbx=observer_params['nbx'],xMin=observer_params['xMin'],xMax=observer_params['xMax'],nby=observer_params['nby'],yMin=observer_params['yMin'],yMax=observer_params['yMax'],nbz = observer_params['nbz'],zMin = observer_params['zMin'],zMax=observer_params['zMax'],highPassFrequency = observer_params['highPassFrequency'])
