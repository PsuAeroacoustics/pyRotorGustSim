...
def filter_loads(geom_params,input_params,res_param,observer_params,acs_params,saved_params,opt = False):
    ...
    def apply_res(res_param,saved_params):
        ...
        N,a,L = get_sample_info(res_param['x'],res_param['N_res'],res_param['OAR'],A_s)
        ...
        Z_tot, alpha = smeared_Z(f,N[filt_ind],a,L,res_param['N_res'],bool(res_param['facesheet']),t_fs = res_param['t_fs'],phi_fs = res_param['phi_fs'],N_fs = res_param['N_fs'],A_r = res_param['OAR'],A_s = A_s[filt_ind],M =res_param['M'],SPL = res_param['SPL'])
        ...
        saved_params.update({'x':res_param['x'],'N':N,'a':a,'L':L,'filt_ind':filt_ind,'Z':Z_tot,'filt_resp':filt_resp,'filt_loads':filt_loads})
    ...
    if opt:
        ...
        r_min, r_max, L_min, L_max = np.min((.5e-3,res_param['r_min'])),np.min((saved_params['sos']/(2*np.pi*f[-1]),res_param['r_max'])),np.max((saved_params['sos']/(f[-1]*4),res_param['L_min'])),np.max((saved_params['sos']/(150*4),res_param['L_max']))
        bounds = get_opt_bounds(res_param['x'],r_min, r_max, L_min, L_max)
        opt_out = differential_evolution(opt_res,x0 = res_param['x'],bounds = bounds, polish=False,maxiter = 10,args = (res_param,geom_params,saved_params))
