...
cases = ['sdof_dist_32','sdof_dist_48','sdof_dist_64','sdof_dist_128']
leglab = []
...
for case in cases:
    ...
    for i,case in enumerate(cases):
        ...
        bvispl_temp[i] = np.round(np.mean(20*np.log10(np.sqrt(np.mean(acs_data['function_values'][:,:,:,-1]**2,axis = -1))/20e-6)),1)
        # bvispl_temp[i] = np.mean(20*np.log10(np.sqrt(acs_data['function_values'][5,:,:,-1]**2)/20e-6))
...
for sweep in sweeps:
    ax.plot(M[sweep],bvispl[sweep], marker = 'o')
ax.set_xlabel(r'$M_t$')
