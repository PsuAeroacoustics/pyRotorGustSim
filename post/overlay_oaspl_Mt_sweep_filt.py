...
par_dir = os.getcwd()
cases_dir = [os.path.join(par_dir,'scaled_Mt_sweep'),os.path.join(par_dir,'scaled_Mt_sweep_sdof_dist')]
...
M = np.array([saved_params[list(oaspl.keys())[0]][case]['omega']*saved_params[list(oaspl.keys())[0]][case]['R']/340 for case in cases])
plot_ind = M.argsort()
...
#     ax.plot(M[plot_ind],(oaspl[list(oaspl.keys())[0]]-oaspl[list(oaspl.keys())[0]])[plot_ind,mic_iter],marker = 'o')
#     ax.set_xlabel(r'$M_t$')
#     ax.set_ylabel(r'$\Delta \ OASPL, \ dB$')
#     ax.set_title(rf'$\\theta = {theta[mic_iter]}^\circ$')
#     ax.grid()
#     plt.savefig(os.path.join(cases_directory,f'd_oaspl_vs_M_{mic_iter}.png'),format = 'png')
